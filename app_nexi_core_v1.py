...
def _conv_comunas_recolectores_activos(empresa_id):
    """Devuelve la unión de comunas declaradas por interesados/recolectores activos."""
    ...
    try:
        ...
        seen=set()
        for row in rows:
            for comuna in _conv_lista((row or {}).get('comunas')):
                c=str(comuna or '').strip()
                k=_conv_norm(c)
                if c and k and k not in seen:
                    seen.add(k)
                    out.append(c)
        return sorted(out,key=lambda x:_conv_norm(x))
    except Exception as e:
        print('NEXI COMUNAS RECOLECTORES ERROR:',repr(e))
        return []
...
def public_conv_config():
    ...
    # Las comunas disponibles para una NUEVA solicitud salen de los
    # recolectores/interesados activos, no de una lista fija.
    comunas_disponibles=_conv_comunas_recolectores_activos(eid)
    c['comunas']=comunas_disponibles
    c['comunas_fuente']='recolectores_activos'
